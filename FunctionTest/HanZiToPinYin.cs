...
, { -10256, "zun" }, { -10254, "zuo" } };
        ...
        public static string ToPinYin(string txt)
        {
            ...
            for (int j = 0; j < arrChar.Length; j++)   //遍历输入的字符 
            {
                ...
                if (charCode > -10254 || charCode < -20319)  //如果不在汉字编码范围内则不改变，查找二级汉字
                {
                    int pos = Array.IndexOf(otherChinese, arrChar[j].ToString());
                    if (pos != decimal.MinusOne)
                    {
                        result.Append(otherPinYin[pos]);
                    }
                    else
                    {
                ...
                else
                {
                    ...
                    if (!CodeCollections.ContainsKey(charCode))
                    {
                        for (int i = charCode; i <= 0; --i)
                        {
                            ...
                            {
                                result.Append(CodeCollections[i].ToUpper());
                                break;
                            }
                        }
                    }
                    ...
                    {
                        result.Append(CodeCollections[charCode].ToUpper());
                    }
                }
            }
